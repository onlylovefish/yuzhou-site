...
## Q&A
1. scheduler中遇到优先级相同的任务怎么执行

  - 按照expirationTime从小到大排序（先到期先执行），如果expirationTime相同，按照创建顺序执行id从小到大
  - 任务存储在最小堆中，最小堆的排序逻辑在```compare```函数中
```js 
...
```
首先按照sortIndex排序，如果相同则按照id排序（升序，先创建的任务id更小）

同优先级任务的timeout相同，```expirationTime=startTime+timeout```,startTime是任务创建时间，因此不同时间创建的expirationTime不同

```js
// 示例
NormalPriority的timeout为5000ms
// 任务A，创建时间1000ms
expirationTime=1000+5000=6000
id=1
// 任务B，创建时间1000ms
expirationTime=1000+5000=6000
id=2
// 任务C，创建时间1001ms
expirationTime=1001+5000=6001
id=3
